...
\item \path|draft -> published| после \path|submit|, если владелец имеет роль \path|author| или \path|admin|
...
\item \path|rejected -> on_moderation| или \path|rejected -> published| при повторном \path|submit| без доработки, в зависимости от роли владельца
...
\item проверяется, что маршрут находится в статусе \path|draft| или \path|rejected|
\item проверяется, что маршрут содержит хотя бы одну точку
\item если владелец маршрута имеет роль \path|user|, статус меняется на \path|on_moderation|
\item если владелец маршрута имеет роль \path|author| или \path|admin|, статус меняется на \path|published|
\end{enumerate}
...
\begin{enumerate}
\item проверяется роль \path|admin|
\item проверяется, что маршрут находится в статусе \path|on_moderation|
\item проверяется, что маршрут содержит хотя бы одну точку
...
\item проверяется, что впечатление находится в статусе \path|draft| или \path|rejected|
\item проверяется, что маршрут впечатления находится в статусе \path|published|
\item если владелец впечатления имеет роль \path|user|, статус меняется на \path|on_moderation|
\item если владелец впечатления имеет роль \path|author| или \path|admin|, статус меняется на \path|published|
\end{enumerate}
...
\begin{enumerate}
\item проверяется роль \path|admin|
\item проверяется, что впечатление находится в статусе \path|on_moderation|
\item проверяется, что маршрут впечатления находится в статусе \path|published|
...
\item система должна предоставлять список городов, в которых есть опубликованные впечатления
\end{enumerate}
...
\tzmethod{GET /showcase/cities}

\tzlabel{Вход}

\begin{enumerate}
\item параметры не требуются
\end{enumerate}

\tzlabel{Что происходит}

\begin{enumerate}
\item система выбирает маршруты, связанные с опубликованными впечатлениями
\item система собирает уникальные города из найденных маршрутов
\item список городов сортируется по названию
\end{enumerate}

\tzlabel{Выход}

\begin{enumerate}
\item массив городов с полями \path|external_city_id| и \path|name|
\end{enumerate}
...
\begin{jsonblock}
{
    ...
    "user_id": 12,
    "impression_id": 55,
    "impression_name": "Историческая прогулка по Москве",
    "status": "paid",
...
\item проверяется, что у пользователя ещё нет покупки этого впечатления в любом статусе
...
\item покупка в статусе \path|failed| может быть оплачена повторно этим же методом
\item повторный вызов для уже оплаченной покупки возвращает покупку без изменений
...
\begin{enumerate}
\item массив покупок пользователя с названием связанного впечатления
\end{enumerate}
...
\begin{enumerate}
\item JSON созданного отзыва с именем пользователя
\end{enumerate}
...
\begin{enumerate}
\item массив отзывов с именами пользователей
\end{enumerate}
...
\item если пользователь явно выбрал город, используется переданный \path|city_external_id|
\item если город явно не выбран, текущий город пользователя определяется по геоданным, которые клиент передаёт в backend, как ближайший город из поддерживаемого справочника
\item если не переданы ни \path|city_external_id|, ни координаты, запрос отклоняется с ошибкой валидации
...
\begin{enumerate}
\item если передан \path|city_external_id|, система использует его
\item иначе текущий город пользователя определяется по переданным координатам как ближайший город из поддерживаемого справочника
\item если не переданы ни город, ни координаты, возвращается ошибка валидации
...
Экран текущего прохождения не требует отдельного BFF-метода: он собирается клиентом из агрегированной карточки впечатления и API прогресса.
...
Все текстовые сообщения об ошибках возвращаются на русском языке. Это относится и к ошибкам валидации, которые формируются внешними библиотеками: например, при некорректном формате email сообщение приводится к виду «Некорректный формат email», а не возвращается в исходном англоязычном виде используемой библиотеки проверки.
...
\item текстовые сообщения об ошибках должны возвращаться на русском языке, включая ошибки валидации, формируемые внешними библиотеками (например, проверкой формата email)
...
\midrule
\path|GET| & \path|/health| & Сервис & Публичный & Проверка доступности сервиса \\
...
\path|GET| & \path|/showcase/cities| & Витрина & Публичный & Получение списка городов с опубликованными впечатлениями \\
...
Исходный код проекта размещён в GitHub-репозитории: \url{https://github.com/NobodyForYouCreator/cursach}.
...
Для взаимодействия с frontend в приложении подключён CORS middleware. Он разрешает локальному клиенту обращаться к backend во время разработки и не меняет внутренние правила авторизации: защищённые API-методы по-прежнему требуют валидный JWT.
...
\subsection{Рекомендации, витрина и аналитика}

Рекомендации реализованы без машинного обучения, что соответствует границам проекта. Система выдаёт опубликованные впечатления по городу и сортирует их по доступным признакам, например рейтингу или популярности. Если клиент передаёт координаты, backend выбирает ближайший город из поддерживаемого справочника с помощью уже используемой функции расчёта расстояния. В справочник включены основные города MVP, включая Москву, Санкт-Петербург, Казань, Екатеринбург, Новосибирск и Калининград.

Для витрины дополнительно реализован API-метод \texttt{GET /showcase/cities}. Он возвращает только те города, для которых есть опубликованные впечатления. Это позволяет frontend строить фильтр витрины по фактическим данным, а не показывать пользователю пустые направления.

В ответах для клиентских экранов используются небольшие денормализованные поля. Список покупок пользователя возвращает название связанного впечатления, а отзывы возвращаются вместе с именем автора отзыва. Эти поля не создают новых сущностей в базе данных, но уменьшают количество дополнительных запросов со стороны frontend.
...
\begin{itemize}
	...
	\item витрина и рекомендации возвращают только опубликованные впечатления
\end{itemize}
...
В ходе курсового проекта был разработан backend-сервис для трэвел-приложения с маршрутами и впечатлениями. Реализация покрывает основные требования технического задания: регистрацию и авторизацию пользователей, роли, маршруты, точки маршрута, впечатления, модерацию, витрину, покупки, отзывы, рекомендации, realtime-прогресс, аналитику, BFF-методы и интеграционный endpoint поиска мест. В реализации также учтены потребности клиентского приложения: включён CORS для локального frontend, добавлен список городов витрины, а ответы покупок и отзывов дополнены данными, которые нужны для отображения экранов без лишних запросов. Сервис имеет единый API-контракт, реляционную модель данных, проверку прав доступа и автоматические интеграционные тесты. Полученный прототип можно использовать как основу для дальнейшей разработки frontend, подключения реальных платёжных систем, внешних картографических API и более сложных рекомендательных механизмов.
...
\begin{longtable}{L{0.24\textwidth}L{0.66\textwidth}}
	...
	\midrule
	\endhead
	\texttt{health.py} & Проверка доступности сервиса. \\
	...
	\texttt{showcase.py} & Публичная витрина опубликованных впечатлений и список городов, в которых есть опубликованные впечатления. \\
	\texttt{purchases.py} & Создание покупки, оплата через заглушку, список покупок пользователя с названием впечатления. \\
	\texttt{reviews.py} & Создание и просмотр отзывов с именами пользователей. \\
	\texttt{recommendations.py} & Rule-based рекомендации по явно выбранному или ближайшему по координатам городу. \\
	...
	\texttt{common.py} & Вспомогательные функции API-слоя: сериализация ответов и фиксация изменений. \\
	\texttt{impression\_details.py} & Сборка детальной карточки впечатления: маршрут, точки, отзывы с именами пользователей, покупка. \\
	\bottomrule
\end{longtable}
